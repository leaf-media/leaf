...
function onKeydown(event) {
  ...
  if (
    ...
    backspaceToDelete.keyupHappened
  ) {
    emit("last-backspace");
  }
...
const datalist = computed(() => {
  return props.autocomplete.filter((a) =>
    a.toLowerCase().startsWith(state.userInput.toLowerCase())
  );
});
...
<template>
  <div class="relative overflow-hidden w-full h-full">
    ...
    <datalist id="autocomplete-list">
      <option :value="item" v-for="item in datalist" :key="item" />
    </datalist>
  </div>
</template>
